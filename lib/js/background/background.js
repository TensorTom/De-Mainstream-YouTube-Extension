...
browser.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  ...
  switch (action) {
    ...
    case SET_ALL:
      ...
      break;
    case SET:
      setChannelEnabled(message.channelId, message.enabled, message['optionalChannelName'] || null);
      sendYouTubeUpdate({
        type: SET,
...
function contextMenuAction(info) {
  if (info !== null && info.hasOwnProperty('menuItemId') && info.hasOwnProperty('selectionText') && info.hasOwnProperty('linkUrl')) {
    ...
    console.log("BLOCK OR UNBLOCK:", channelId, channelName, enabled);
    ...
    sendYouTubeUpdate({
      type: SEND_SUCCESS,
      msg: 'Successfully ' + action + " channel: " + channelName
    });
  }
}
